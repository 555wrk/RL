...
class DualArmGrasper:
    ...
    def execute_grasp(self):
        ...
        self.robot_left = self.env.robot_left  # 获取左臂实例
        self.robot_right = self.env.robot_right  # 获取右臂实例
    ...
    def move_to_target(self, left_target, right_target, gripper_state=1.0, max_steps=250):
        ...
        for step in range(max_steps):
            ...
            left_displacement = np.clip(left_displacement, -step_size, step_size)  # 修改：使用步长限制位移

            # 计算右臂位移
            right_displacement = right_target - right_ee_pos
            right_displacement = np.clip(right_displacement, -step_size, step_size)  # 修改：使用步长限制位移

            # 计算左臂误差
            left_distance = np.linalg.norm(left_displacement)

            # 计算右臂误差（重点优化）
            right_distance = np.linalg.norm(right_displacement)
            ...
            action = np.concatenate([
                left_displacement* 5, [gripper_state],
                right_displacement* 5, [gripper_state]
            ])
            ...
            # 执行动作
            self.robot_left.osc(left_displacement)  # 调用新的操作空间控制方法
            self.robot_right.osc(right_displacement)  # 调用新的操作空间控制方法
            self._set_gripper_state(gripper_state)

            p.stepSimulation()
            time.sleep(self.env._timeStep)
    ...
    def _set_gripper_state(self, gripper_state):
        ...
        self.robot_left.gripper_control(gripper_state)  # 调用新的夹爪控制方法
        self.robot_right.gripper_control(gripper_state)  # 调用新的夹爪控制方法
    ...
    def save_expert_data(self):
        # 先读取现有数据（如果有）
        existing_data = []
        if os.path.exists('expert_data.pkl1'):
            with open('expert_data.pkl1', 'rb') as f:
                while True:
                    try:
