...
class MADDPG:
    ...
    def update_parameters(self, memory, batch_size, updates, bc_weight=0.5):
        # 从经验池采样
        state_batch, action_batch, reward_batch, next_state_batch, mask_batch, is_expert = memory.sample(batch_size)
        ...
        is_expert = torch.BoolTensor(is_expert).to(self.device)
        ...
        for i, actor in enumerate(self.actors):
            ...
            if is_expert.any():
                ...
                expert_agent_actions = expert_actions[:,
                                       sum(self.action_dims[:i]):sum(self.action_dims[:i + 1])]

                policy_actions = actor(expert_states)
                bc_loss = F.mse_loss(policy_actions, expert_agent_actions)
                actor_loss += bc_weight * bc_loss
