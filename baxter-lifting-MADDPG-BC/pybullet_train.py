...
def parse_args():
    ...
    parser.add_argument('--policy', default="Gaussian",
                        help='策略类型: Gaussian | Deterministic (默认: Gaussian)')
    ...
    parser.add_argument('--expert_data', default="baxter_bullet_env/expert_data.pkl", help='专家数据文件路径')
...
if __name__ == "__main__":
    ...
    agent = SAC(23, env.action_space, args)  # 直接传入实际维度23
            ...
            while not done:
                ...
                if len(memory) > args.batch_size:
                    ...
                    for i in range(args.updates_per_step):
                        critic_1_loss, critic_2_loss, policy_loss, bc_loss, alpha = agent.update_parameters(
                            memory, args.batch_size, updates, bc_weight=current_bc_weight
                        )
                        writer.add_scalar('loss/critic_1', critic_1_loss, updates)
                        writer.add_scalar('loss/critic_2', critic_2_loss, updates)
                        writer.add_scalar('loss/policy', policy_loss, updates)
                        writer.add_scalar('loss/bc_loss', bc_loss, updates)  # 新增：记录BC损失
                        writer.add_scalar('loss/total_policy', policy_loss + current_bc_weight * bc_loss, updates)
