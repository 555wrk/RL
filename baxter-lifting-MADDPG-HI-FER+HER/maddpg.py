...
class MADDPG:
    ...
    def select_action(self, state, goal, agent_idx, evaluate=False):
        state = torch.FloatTensor(state).to(self.device).unsqueeze(0)
        goal = torch.FloatTensor(goal).to(self.device).unsqueeze(0)
        if evaluate:
            return self.actors[agent_idx](state, goal).detach().cpu().numpy()[0]
        else:
            return self.actors[agent_idx].sample(state, goal)[0].detach().cpu().numpy()[0]

    def update_parameters(self, state_batch, action_batch, reward_batch, next_state_batch, mask_batch, goal_batch):
        # 转换为张量
        ...
        with torch.no_grad():
            # 目标演员动作（需要传入目标）
            next_left_action = self.actors[0](next_state_batch, goal_batch)
            next_right_action = self.actors[1](next_state_batch, goal_batch)
            next_actions = torch.cat([next_left_action, next_right_action], dim=1)

            # 目标评论家Q值 - 关键修复：取两个Q值的最小值（同时传入目标）
            target_q1, target_q2 = self.critic_target(next_state_batch, next_actions, goal_batch)
            target_q = torch.min(target_q1, target_q2)
            next_q_value = reward_batch + mask_batch * self.gamma * target_q

        # 更新评论家（传入目标）
        current_q1, current_q2 = self.critic(state_batch, action_batch, goal_batch)
        ...
        for i, actor in enumerate(self.actors):
            # 当前演员动作（传入目标）
            current_action = actor(state_batch, goal_batch)
            ...
            # 演员损失 - 使用第一个Q值计算（传入目标）
            actor_q1, _ = self.critic(state_batch, actions, goal_batch)
    ...
    def reset_parameters(self):
        ...
        for optimizer in self.actor_optimizers:
            for param_group in optimizer.param_groups:
                param_group['lr'] = self.args.lr  # 使用保存的args
...
class CentralizedCritic(nn.Module):
    def __init__(self, num_inputs, total_action_dim, hidden_size):
        ...
        # 输入：状态（23）+ 动作（8）+ 目标（3）=34
        self.input_dim = num_inputs + total_action_dim + 3
    ...
    def forward(self, state, action, goal):
        # 拼接状态、动作和目标
        sa = torch.cat([state, action, goal], dim=1)
