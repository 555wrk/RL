...
from replay_memory import ParallelReplayMemory
import signal
import sys
...
def parse_args():
    ...
    parser.add_argument('--replay_ratio', type=int, default=8, help='经验回放比率')
    parser.add_argument('--reset_frequency', type=int, default=100000, help='网络重置频率')
    parser.add_argument('--main_batch_size', type=int, default=256, help='主缓冲区批次大小')
    parser.add_argument('--temp_batch_size', type=int, default=64, help='临时缓冲区批次大小')
...
if __name__ == "__main__":
    ...
    # TensorBoard日志
    log_folder_name = f"log/MADDPG_{args.env_name}_{args.seed}_{args.policy}_{current_time}"
    writer = SummaryWriter(log_folder_name)

    # 初始化并行经验回放
    memory = ParallelReplayMemory(args.replay_size, 1000, args.seed)  # 临时缓冲区固定为1000
    ...
    try:
        for i_episode in range(1, int(args.end_epoch + 1)):
            ...
            state = env.reset()
            goal = env.get_goal()  # 获取目标

            while not done:
                # 获取当前状态下的achieved_goal（物体位置）
                achieved_goal = state[6:9]  # 根据状态定义，物体位置在索引6:9

                if args.start_steps > total_numsteps:
                    action = env.action_space.sample()  # 随机动作
                else:
                    # 双臂动作选择，传入状态和目标
                    left_action = agent.select_action(state, goal, agent_idx=0, evaluate=False)
                    right_action = agent.select_action(state, goal, agent_idx=1, evaluate=False)
                    action = np.concatenate([left_action, right_action])

                # 与环境交互
                next_state, reward, done, info = env.step(action)
                next_achieved_goal = next_state[6:9]  # 下一步的achieved_goal

                # 注意：这里我们使用mask_batch来区分终止状态（非因时间限制）
                mask = 1 if episode_steps == env.max_episode_steps else float(not done)

                # 存入经验回放（包括目标、achieved_goal和next_achieved_goal）
                memory.push(state, action, reward, next_state, mask, goal, achieved_goal, next_achieved_goal)
                ...
                # 重复训练：每次交互后多次更新网络
                if len(memory.main_buffer) > args.main_batch_size and len(memory.temp_buffer) > args.temp_batch_size:
                    for _ in range(args.replay_ratio):
                        # 从双缓冲区采样（现在返回6个元素，包括goal_batch）
                        state_batch, action_batch, reward_batch, next_state_batch, mask_batch, goal_batch = memory.sample(
                            args.main_batch_size, args.temp_batch_size
                        )

                        # 更新网络参数（传入goal_batch）
                        critic_loss, left_actor_loss, right_actor_loss = agent.update_parameters(
                            state_batch, action_batch, reward_batch, next_state_batch, mask_batch, goal_batch
                        )
                        ...
                        # 周期性遗忘机制
                        if total_updates % args.reset_frequency == 0:
                            print(f"Resetting networks at update {total_updates}")
            ...
            if i_episode % 20 == 0 and args.eval:
                ...
                for ep in range(episodes):
                    ...
                    while not done and test_step < args.epoch_step:
                        left_action = agent.select_action(state, goal, agent_idx=0, evaluate=True)
                        right_action = agent.select_action(state, goal, agent_idx=1, evaluate=True)
                        action = np.concatenate([left_action, right_action])
