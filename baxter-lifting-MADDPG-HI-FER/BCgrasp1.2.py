...
class DualArmGrasper:
    def __init__(self, env):
        ...
        self.grasp_height = 0.105 # 抓取高度（低于物体顶部）
        self.lift_height = 0.15 # 提升高度
        self.success_threshold = 0.1  # 成功抓取的最小高度

        # 右臂特定参数（解决右臂精度问题）
        self.right_arm_max_velocity = 0.05  # 右臂最大速度降低
        self.tolerance = 0.015  # 容差放宽到2cm
        self.min_distance = 0.005  # 最小移动距离
    ...
    def move_to_target(self, left_target, right_target, gripper_state=1.0, max_steps=250):
        """移动双臂到目标位置（右臂专用优化）"""
        left_integral = np.zeros(3)  # 左臂积分项
        right_integral = np.zeros(3)  # 右臂积分项
        left_prev_error = np.zeros(3)  # 左臂前次误差
        right_prev_error = np.zeros(3)  # 右臂前次误差
        ...
        for step in range(max_steps):
            ...
            # 左臂控制（标准PID）
            left_integral += left_error * 0.1
            left_derivative = left_error - left_prev_error
            left_action = (
                    3 * left_error +  # P项
                    0.1 * left_integral +  # I项
                    0.05 * left_derivative  # D项
            )
            left_action = np.clip(left_action, -0.1, 0.1)
            left_prev_error = left_error

            # 右臂控制（增强控制）
            right_integral += right_error * 0.1  # 更强的积分项
            #right_integral = np.clip(right_integral, -0.05, 0.05)
            right_derivative = right_error - right_prev_error

            right_action = (
                    3.5 * right_error +  # 更强的P项
                    0.1 * right_integral +  # 更强的I项
                    0.05 * right_derivative  # 更强的D项
            )
